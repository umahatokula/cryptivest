...
contract Staking {
  ...
  function stakeEther(uint numDays) external payable {
    require(tiers[numDays] > 0, "Mappping not found");
  ...
  function calculateInterest(uint basisPoint, uint numDays, uint weiAmount) private pure returns (uint) {
    return basisPoint * weiAmount / 10000; // 700 / 1000 = 0.07
  }
  ...
  function closePosition(uint positionId) external {
    require(positions[positionId].walletAddres == msg.sender, "Only position creator may modify position");
